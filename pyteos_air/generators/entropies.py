...
def compute_thermo(num_procs, test):
    """
    \b
    This script computes two data files:
        massfraction_air_pottempequi_g_common_pressures.dat  
        massfraction_air_pottemp_g_ref_common_pressures.dat
    ...
    compute_pottempequi(num_procs, thermo_axes, name)
    thermo_axes['pref'] = 1e5
    compute_pottemp(num_procs, thermo_axes, name)


def compute_pottempequi(num_procs, thermo_axes, name):
    func_list = {}
    ...
    with open('_'.join(func_list.keys()) + '_' + realm + '_' + name + '.dat', 'w') as file:
...
def compute_pottemp(num_procs, thermo_axes, name):
    func_list = {}
    functions=['pottemp']
    realm='g_ref'
